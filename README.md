...
| Subject index | docs.legis subject index | headings as printed; each entry attributed to the heading its document path names (the index's continuation pages overlap), matched by exact session and identifier |
| Hearing recordings | WisconsinEye | metadata matched by exact date and committee title; links out, never hosted |
| Candidates and election results (legislature, statewide offices, U.S. House) | Wisconsin Elections Commission | before the primary, the ballot-access report, every row accounted for against each office's printed subtotal; after it, each party's nominee from the certified primary canvass under Wis. Stat. 8.16 plus the independents the Commission approved, with anything the records cannot settle shown as open (rulings with an official basis in `importer/wec_rulings.json`); certified ward-by-ward canvasses for results |
| Statewide ballot questions | Commission referendum notice; docs.legis enrolled resolutions | each question kept only when its enrolled resolution states it for that election, names a first consideration whose history agrees, and carries the constitution text word for word as the preceding Legislature approved it (`importer/import_amendments.py`) |
