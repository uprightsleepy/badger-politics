...
function fixture(t) {
  ...
  t.after(() => conn.close());
  conn.exec(schema);
  for (const [tenant, negative] of [["old", "No"], ["new", "Nay"]]) {
    ...
    conn.prepare("INSERT INTO local_events VALUES (?, 1, '2026-01-01', 'Final', 'https://example.invalid/meeting')")
      .run(tenant);
    const casts = [
      [1, 1, [[1, negative], [2, "Aye"], [3, "Aye"]]],
      [2, 0, [[1, "Aye"], [2, negative], [3, negative]]],
      [3, 1, [[1, "Abstained"], [2, "Aye"], [3, "Aye"]]],
      [4, 0, [[9, negative], [2, "Aye"], [3, "No"]]],
      [5, null, [[1, negative]]],
    ];
...
test("No and Nay yield identical totals without changing recorded votes or mixing cities", t => {
  const { api } = fixture(t);
  for (const [tenant, negative] of [["old", "No"], ["new", "Nay"]]) {
    ...
    assert.deepEqual(api.localMemberVotes(tenant, 1, 10).map(v => v.value),
      [negative, "Abstained", "Aye", negative]);
...
test("negative vote labels share a display style without classifying other positions as no", () => {
  ...
  for (const value of ["Aye", "Abstained", "Presiding", "No Vote", "Unknown"]) {
