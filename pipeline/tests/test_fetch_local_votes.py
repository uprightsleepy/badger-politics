...
def source(tmp_path, monkeypatch):
    ...
    def call(http, tenant, path, **params):
        calls.append(path)
...
def test_backfill_advances_and_preserves_existing_bytes(source):
    ...
    assert fetch.fetch_tenant(None, MADISON, [-1]) == (5, 0)
    ...
    calls.clear()
    assert fetch.fetch_tenant(None, MADISON, [-1]) == (4, 5)
    ...
    events.pop()
    fetch.fetch_tenant(None, MADISON, [-1])
...
def test_draft_refresh_does_not_starve_new_meetings(source):
    ...
    assert fetch.fetch_tenant(None, MADISON, [-1]) == (5, 0)
    calls.clear()
    assert fetch.fetch_tenant(None, MADISON, [-1]) == (9, 0)
...
def test_first_batch_includes_reviewed_rollcall_then_newest_meetings(source):
    ...
    assert fetch.fetch_tenant(None, spec, [-1]) == (5, 0)
...
def test_global_budget_and_future_meetings(source):
    ...
    assert fetch.fetch_tenant(None, MADISON, [2]) == (2, 0)
    coverage = json.loads((archive / "coverage.json").read_text())
...
def test_existing_tenants_still_use_final_and_have_no_new_limit(source):
    ...
    assert fetch.fetch_tenant(None, spec, [-1]) == (9, 0)
    # Approved is not Milwaukee's settled status.
    assert fetch.fetch_tenant(None, spec, [-1]) == (9, 0)
    for event in events:
        event["EventMinutesStatusName"] = "Final"
    fetch.fetch_tenant(None, spec, [-1])
    calls.clear()
    assert fetch.fetch_tenant(None, spec, [-1]) == (0, 9)
    assert not any(p.endswith("/EventItems") for p in calls)
...
def test_truncated_meeting_response_keeps_cached_record(source, monkeypatch):
    ...
    def truncated(http, tenant, path, **params):
        if path.endswith("/EventItems"):
            return [{}] * fetch.PAGE
        return real_call(http, tenant, path, **params)
...
def test_cli_selects_cities_and_separate_archive(monkeypatch, tmp_path):
    ...
    def collect(http, spec, budget, data_dir):
